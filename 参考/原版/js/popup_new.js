  ...
  sync = {
    ...
    bindEvent: function () {
      ...
      $('#btnTest').on('click', async () => {
        chrome.runtime.sendMessage('unBingEvents');
      });

      $('#btnClean').on('click', async () => {
        // that.utils.Bookmark.emptyBookmarks();
        that.utils.Store.setState({ isUserActionEvent: true });
      ...
      $('#btnDownload').on('click', async function () {
        ...
        if (bookmarks) {
          await that.utils.Bookmark.emptyBookmarks();

          // 等所有书签都清空后开始设置书签
          await that.utils.Bookmark.setBookmarks('1', bookmarks);
          $('#successWrap').show();
          $('.successText').text('Download Success');

          chrome.runtime.sendMessage('bingEvents');
        } else {
