...
const Gitee = {
  ...
  updateGit: async function (options, bookmarksData, sha) {
    try {
      ...
      const res = await $.ajax({ type: 'PUT', url, data, crossDomain: true });
...
const Store = {
  ...
  getState: function (stateKey) {
    return new Promise((resolve, reject) => {
      chrome.storage.local.get([stateKey], obj => {
        if (chrome.runtime.lastError) {
          reject(chrome.runtime.lastError);
        } else {
          if (obj && obj.hasOwnProperty(stateKey)) resolve(obj[stateKey]);
          resolve(false);
        }
      });
    });
  ...
  setState: function (state) {
    return new Promise((resolve, reject) => {
      let defaultState = { isUserActionEvent: state };
...
const Message = {
  alter: function (message, title = '弹窗消息') {
    return new Promise(resolve => {
      chrome.notifications.create({
        type: 'basic',
        title,
        message,
        iconUrl: 'icon/icon48_cps.png',
      });
      resolve(true);
    });
  },
